...
Promise.all(promises).then(function ready(values) {
  ...
  var num_legend = 9;
  const legendScale = d3.scaleLinear().domain([0, 1]).range([0.4, 1]);
  ...
  legenditem
    ...
    .style("fill", function (d, i) {
      return d3.interpolateOranges(legendScale(i / num_legend));
    });
  ...
  legenditem
    ...
    .text(function (d, i) {
      if (i % 2 == 0) {
        var text = maps_formatTicks((max_cases * i) / num_legend);
      } else {
        var text = "";
      }
      return text;
  ...
  function mouseover(event, d) {
    ...
    if ("values" in d) {
      if (Array.from(d.values.keys()).includes(dates_range[idx])) {
        ...
        bodyData = [
          ["Population", maps_formatTicks(TipData.population)],
          [
            "Cases (per Million)",
            maps_formatTicks(TipData.total_cases_per_million),
          ],
          [
            "Deaths (per Million)",
            maps_formatTicks(TipData.total_deaths_per_million),
          ],
          [
            "Tests (per Thousand)",
            maps_formatTicks(TipData.total_tests_per_thousand),
          ],
          [
            "Vaccinations (per Hundred)",
            maps_formatTicks(TipData.total_vaccinations_per_hundred),
          ],
        ];
    ...
    countryShapes.style("fill", function (d) {
      if ("values" in d) {
        if (Array.from(d.values.keys()).includes(year)) {
          ...
          // return color(value);
          return d3.interpolateOranges(legendScale(value / max_cases));
        }
      }
      return d3.interpolateOranges(legendScale(0));
    });
